...
#             print("\t"+y)
#             break
#     print(x[:-2]+"-inst.ll: "+ x[:-1]+"ll\t" +   "\n\topt -load ./LLVMinstkern.so -analyze -inst fs/ext4/"+x[:-1]+"ll  -o  fs/ext4/"+ x[:-2]+"-inst.ll")
#     print(x+": " + x[:-2] + "-inst.ll  \t\n\tllc -code-model=kernel --filetype=obj fs/ext4/"+x[:-1] + "ll  \n")
